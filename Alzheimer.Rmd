...
would be influential. The result suggest that MMSE test result, age in years and
...
equates head size causing the ASF to be proportional to 
TIV[3](Buckner%20et%20al.,%202004)). I decided not to keep the `ASF` variable in
my model.
...
In the summary below, with the standard error of `nWBV` being 16.5, it is 
impossible to claim that `nWBV` is an important factor for prediction. And the 
reason is probably because `nWBV` is correlated with Age and MMSE.

```{r fig.cap= "summary of mod2"}
summary(mod2, 
...
Below is a part of summary of random effects of patients.

```{r fig.cap= "First 5 random effects esitmate from summary of final model"}
summary(mod2, 
        pars = c( "b[(Intercept) pid:OAS2_0007]","b[(Intercept) pid:OAS2_0001]","b[(Intercept) pid:OAS2_0005]","b[(Intercept) pid:OAS2_0004]", "b[(Intercept) pid:OAS2_0002]"),
        probs = c(0.025, 0.975),
        digits = 2)
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.cap = "Figure 7 model check"}
# model checking
